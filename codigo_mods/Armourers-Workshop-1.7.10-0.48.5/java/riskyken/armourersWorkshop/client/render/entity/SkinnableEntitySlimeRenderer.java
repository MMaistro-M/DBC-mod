...
public class SkinnableEntitySlimeRenderer
implements ISkinnableEntityRenderer<EntitySlime> {
    @Override
    public void render(EntitySlime entity, RendererLivingEntity renderer, double x, double y, double z, IEntityEquipment entityEquipment) {
        ...
        float f1 = entity.func_70809_q();
        float f2 = (entity.field_70812_c + (entity.field_70811_b - entity.field_70812_c) * ModClientFMLEventHandler.renderTickTime) / (f1 * 0.5f + 1.0f);
        float f3 = 1.0f / (f2 + 1.0f);
        GL11.glScalef((float)(f3 * f1), (float)(1.0f / f3 * f1), (float)(f3 * f1));
        GL11.glTranslated((double)0.0, (double)(-0.12f * scale), (double)0.0);
