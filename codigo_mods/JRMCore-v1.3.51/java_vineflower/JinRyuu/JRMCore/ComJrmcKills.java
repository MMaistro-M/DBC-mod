...
public class ComJrmcKills extends CommandBase {
   ...
   public void func_71515_b(ICommandSender par1ICommandSender, String[] par2ArrayOfStr) {
      ...
      if (cur <= 1 || flagPlayer) {
         ...
         int KllCN = JRMCoreH.getInt(entityplayermp, "jrmcKillCountNeut");
         int KllCE = JRMCoreH.getInt(entityplayermp, "jrmcKillCountEvil");
         ...
         this.notifyAdmins(
            par1ICommandSender,
            "%s checked kill counts of %s  ",
            new Object[]{func_71521_c(par1ICommandSender).getDisplayName(), entityplayermp.func_70005_c_()}
         );
      } else if (flagTop) {
         ...
         if (Karma > 0) {
            entitycommansender.func_145747_a(
               new ChatComponentText("Most Wanted: " + topBadKarma[1] + " with " + topBadKarma[0] + " bad karma").func_150255_a(color)
            );
         }

         if (Total > 0) {
            entitycommansender.func_145747_a(
               new ChatComponentText("Top Total Kills: " + topTotalKills[1] + " with " + topTotalKills[0] + " kill count").func_150255_a(color)
            );
         }

         if (Good > 0) {
            entitycommansender.func_145747_a(
               new ChatComponentText("Top Good Kills: " + topGoodKills[1] + " with " + topGoodKills[0] + " kill count").func_150255_a(color)
            );
         }

         if (Neutral > 0) {
            entitycommansender.func_145747_a(
               new ChatComponentText("Top Neutral Kills: " + topNeutralKills[1] + " with " + topNeutralKills[0] + " kill count").func_150255_a(color)
            );
         }

         if (Evil > 0) {
            entitycommansender.func_145747_a(
               new ChatComponentText("Top Evil Kills: " + topEvilKills[1] + " with " + topEvilKills[0] + " kill count").func_150255_a(color)
            );
         }
