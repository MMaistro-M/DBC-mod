...
public class JRMCoreGuiSCM extends GuiScreen {
   ...
   public void func_73863_a(int x, int y, float f) {
      ...
      if (this.guiID == 11) {
         ...
         this.field_146292_n
            .add(
               new JRMCoreGuiButtons00(
                  102, posX + 50, posY + 85, 80, 20, JRMCoreH.Pwrtyp == 1 ? JRMCoreH.trl("jrmc", "KiTechniques") : JRMCoreH.trl("jrmc", "Jutsu"), 0
               )
            );
         ...
         if (JRMCoreH.PlyrSkillX != null && !JRMCoreH.PlyrSkillX.contains("pty") && JRMCoreH.PlyrSkillX.length() > 1) {
            b++;
            String name = JRMCoreH.trl(mod, JRMCoreH.SklName(JRMCoreH.PlyrSkillX, rSkls, rSklsNms, JRMCoreH.Race));
            int n = Integer.parseInt(JRMCoreH.PlyrSkillX.substring(2));
            var8.func_78276_b("§0" + name + (n < 6 ? " §8lvl: " + n : ""), guiLeft + 5, guiTop + 15 + b * 10, 0);
            ...
            if ((JRMCoreH.Race == 1 || JRMCoreH.Race == 2) && JRMCoreH.PlyrSkillY != null && JRMCoreH.PlyrSkillY.contains("Sai") && n >= 5) {
               this.field_146292_n
                  .add(
                     new JRMCoreGuiButtonsA3(
                        JRMCoreH.PlyrSkillY.contains("R") ? 343 : 342,
                        guiLeft + 10 + var8.func_78256_a(name + (n < 6 ? " §8lvl: " + n : "")),
                        guiTop + 15 + b * 10,
                        20,
                        JRMCoreH.PlyrSkillY.contains("R") ? 0 : 1
                     )
                  );
            }
            ...
            if (JRMCoreH.Race != 1 && JRMCoreH.Race != 2 && JRMCoreH.Race != 4) {
               String s = JRMCoreH.trl("jrmc", "NoUpgrade");
               var8.func_78276_b(s, guiLeft + 250 - var8.func_78256_a(s), guiTop + 15 + b * 10, 0);
            } else {
               String s = n < (JRMCoreH.Race != 1 && JRMCoreH.Race != 2 ? (JRMCoreH.Race == 4 ? 4 : 0) : 6)
                  ? (tpLevelCost == -1 ? JRMCoreH.trl("jrmc", "UpgradeLocked") : "TP: " + tpLevelCost)
                  : JRMCoreH.trl("jrmc", "Maxed");
               var8.func_78276_b(s, guiLeft + 250 - var8.func_78256_a(s), guiTop + 15 + b * 10, 0);
            }
         }

         if (JRMCoreH.PlyrSkillY != null
            && !JRMCoreH.PlyrSkillY.contains("pty")
            && !JRMCoreH.PlyrSkillY.contains("Sai")
            && JRMCoreH.Race != 1
            && JRMCoreH.Race != 2
            && JRMCoreH.PlyrSkillY.length() > 1) {
            b++;
            String name = JRMCoreH.SklName(JRMCoreH.PlyrSkillY, cSkls, cSklsNms);
            int n = Integer.parseInt(JRMCoreH.PlyrSkillY.substring(2));
            var8.func_78276_b(
               "§0" + (JRMCoreH.Race != 1 && JRMCoreH.Race != 2 ? name + " §8lvl: " + (n + 1) : JRMCoreH.TransSaiUpNam[n]),
               guiLeft + 5,
               guiTop + 15 + b * 10,
               0
            );
            ...
            String s = n <= 8 ? (tpLevelCost == -1 ? JRMCoreH.trl("jrmc", "UpgradeLocked") : "TP: " + tpLevelCost) : JRMCoreH.trl("jrmc", "Maxed");
            var8.func_78276_b(s, guiLeft + 250 - var8.func_78256_a(s), guiTop + 15 + b * 10, 0);
         }
         ...
         if (JRMCoreH.PlyrSkills != null) {
            ...
            for (byte i = 0; i < JRMCoreH.PlyrSkills.length; i++) {
               ...
               if (!currentSkill.contains("pty") && currentSkill.length() > 2) {
                  b++;
                  String name = JRMCoreH.Pwrtyp == 1
                     ? JRMCoreH.trl("dbc", JRMCoreH.SklName(currentSkill, skls, sklsNms))
                     : JRMCoreH.trl("nc", JRMCoreH.SklName(currentSkill, skls, sklsNms));
                  int n = Integer.parseInt(currentSkill.substring(2));
                  var8.func_78276_b("§0" + name + " §8lvl: " + (n + 1), guiLeft + 5, guiTop + 15 + b * 10, 0);
      ...
      if (this.guiID == 16) {
         ...
         for (byte i = 0; i < nms; i++) {
            if (i2 <= 13 + this.ipg * 13 && i2 >= 0 + this.ipg * 13) {
               ...
               if (JRMCoreH.SklLvl(i, pwr) < 1) {
                  ...
                  String n = " " + tpCost + " tp";
                  int nw = this.field_146289_q.func_78256_a(fn);
                  if (tpCost != -1 && JRMCoreH.curTP >= tpCost) {
                     this.field_146292_n
                        .add(
                           new JRMCoreGuiButtons01(
                              JRMCoreH.SklLvl(i, pwr) < 1 ? 4000 + i : -1,
                              guiLeft + xSize / 2 - 122 + fnw + onw,
                              guiTop + (ySize + 1) / 2 - 64 + i * 10 - this.lp * 13 * 10,
                              nw,
                              n,
                              JRMCoreH.techNCCol[1]
                           )
                        );
                  } else {
                     var8.func_78276_b(
                        n, guiLeft + xSize / 2 - 122 + fnw + onw, guiTop + (ySize + 1) / 2 - 64 + i * 10 - this.lp * 13 * 10, JRMCoreH.techNCCol[0]
                     );
                  }
               }
            }
         ...
         if (i2 > 13 + this.lp * 13) {
            ...
            this.field_146292_n
               .add(new JRMCoreGuiButtons00(88, guiLeft + xSize / 2 + 130, guiTop + (ySize + 1) / 2 + 15, this.field_146289_q.func_78256_a(n) + 8, 20, n, 0));
         }
