   ...
   public void func_71515_b(ICommandSender par1ICommandSender, String[] par2ArrayOfStr) {
      ...
      if (entitycommansenderName.equals("Console")) {
         ...
         if (par2ArrayOfStr.length > 1) {
            if (!JRMCoreConfig.JRMCCheckOnOthersWithoutOP
               && entityplayermp != null
               && !entitycommansenderName.equals("Console")
               && !par2ArrayOfStr[1].equals(entitycommansenderName)) {
               sheet = false;
               ChatStyle color = new ChatStyle().func_150238_a(EnumChatFormatting.RED);
               entityplayermp.func_145747_a(
                  new ChatComponentText("JRMCCheck - Non OP Players can check other Player's Sheet is DISABLED!").func_150255_a(color)
               );
            }
                  ...
                  entitycommansender.func_145747_a(
                     new ChatComponentText(
                           "Skill: lvl "
                              ...
                              + " "
                              + JRMCoreH.SklName(skl, JRMCoreH.DBCSkillsIDs, JRMCoreH.DBCSkillNames)
                        )
                        .func_150255_a(colorG)
                  );
               }
            }
