        ...
        while (pos < len) {
            ...
            if (Character.isDigit(c) || c == '.' && pos + 1 < len && Character.isDigit(expr.charAt(pos + 1))) {
                ...
                if (c == '0' && pos + 1 < len) {
                    ...
                    if (next == 'x' || next == 'X') {
                        pos += 2;
                        while (pos < len && ExpressionTokenizer.isHexDigit(expr.charAt(pos))) {
                            ++pos;
                        }
                    ...
                    if (next == 'b' || next == 'B') {
                        pos += 2;
                        while (pos < len && (expr.charAt(pos) == '0' || expr.charAt(pos) == '1')) {
                            ++pos;
                        }
                        tokens.add(new ExpressionToken(ExpressionToken.TokenKind.INT_LITERAL, expr.substring(start, pos), start, pos));
                        continue;
                ...
                if (pos < len) {
                    ...
                    if (suffix == 'f' || suffix == 'F') {
                        kind = ExpressionToken.TokenKind.FLOAT_LITERAL;
                        ++pos;
                    } else if (suffix == 'd' || suffix == 'D') {
                        kind = ExpressionToken.TokenKind.DOUBLE_LITERAL;
                        ++pos;
                    } else if (suffix == 'L' || suffix == 'l') {
                        kind = ExpressionToken.TokenKind.LONG_LITERAL;
                        ++pos;
                    } else if (hasDecimal) {
                        kind = ExpressionToken.TokenKind.DOUBLE_LITERAL;
                    }
            ...
            if (Character.isJavaIdentifierStart(c)) {
                start = pos;
                while (pos < len && Character.isJavaIdentifierPart(expr.charAt(pos))) {
                    ++pos;
                }
            ...
            if (pos + 3 <= len && (">>>".equals(s3 = expr.substring(pos, pos + 3)) || "<<=".equals(s3) || ">>=".equals(s3))) {
                op = s3;
            }
            if (op == null && pos + 4 <= len && ">>>=".equals(s4 = expr.substring(pos, pos + 4))) {
                op = s4;
            }
            if (op == null && pos + 2 <= len && ("++".equals(s2 = expr.substring(pos, pos + 2)) || "--".equals(s2) || "+=".equals(s2) || "-=".equals(s2) || "*=".equals(s2) || "/=".equals(s2) || "%=".equals(s2) || "&=".equals(s2) || "|=".equals(s2) || "^=".equals(s2) || "==".equals(s2) || "!=".equals(s2) || "<=".equals(s2) || ">=".equals(s2) || "&&".equals(s2) || "||".equals(s2) || "<<".equals(s2) || ">>".equals(s2))) {
                op = s2;
            }
            if (op == null) {
                switch (c) {
            ...
            if (op != null) {
                tokens.add(ExpressionToken.operator(op, start, pos += op.length()));
                continue;
            }
            switch (c) {
                case '(': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.LEFT_PAREN, "(", pos, pos + 1));
                    break;
                }
                case ')': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.RIGHT_PAREN, ")", pos, pos + 1));
                    break;
                }
                case '[': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.LEFT_BRACKET, "[", pos, pos + 1));
                    break;
                }
                case ']': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.RIGHT_BRACKET, "]", pos, pos + 1));
                    break;
                }
                case '{': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.LEFT_BRACE, "{", pos, pos + 1));
                    break;
                }
                case '}': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.RIGHT_BRACE, "}", pos, pos + 1));
                    break;
                }
                case '.': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.DOT, ".", pos, pos + 1));
                    break;
                }
                case ',': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.COMMA, ",", pos, pos + 1));
                    break;
                }
                case '?': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.QUESTION, "?", pos, pos + 1));
                    break;
                }
                case ':': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.COLON, ":", pos, pos + 1));
                    break;
                }
                case ';': {
                    tokens.add(new ExpressionToken(ExpressionToken.TokenKind.SEMICOLON, ";", pos, pos + 1));
                }
            }
            ++pos;
        }
