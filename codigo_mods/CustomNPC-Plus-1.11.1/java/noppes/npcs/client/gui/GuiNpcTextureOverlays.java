...
extends GuiNpcSelectionInterface {
    ...
    public void func_73866_w_() {
        String asset;
        super.func_73866_w_();
        String str = this.npc.display.skinOverlayData.overlayList.containsKey(0) ? this.npc.display.skinOverlayData.overlayList.get(0).getTexture() : "";
        int index = str.lastIndexOf("/");
        if (index > 0 && str.equals(this.assets.getAsset(asset = str.substring(index + 1)))) {
    ...
    public void elementClicked() {
        if (this.dataTextures.contains(this.slot.selected) && this.slot.selected != null) {
            this.npc.display.skinOverlayData.overlayList.put(0, new SkinOverlay(this.assets.getAsset(this.slot.selected)));
        }
    }
