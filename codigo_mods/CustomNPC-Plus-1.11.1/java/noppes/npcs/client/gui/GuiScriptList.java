...
extends SubGuiInterface {
    ...
    public void func_73866_w_() {
        ...
        temp.removeAll(this.scriptUnit.getExternalScripts());
        this.scroll1.setList(temp);
        this.scroll2.setList(this.scriptUnit.getExternalScripts());
    ...
    protected void func_146284_a(GuiButton guibutton) {
        ...
        if (button.field_146127_k == 1 && this.scroll1.hasSelected()) {
            this.scriptUnit.getExternalScripts().add(this.scroll1.getSelected());
            this.scroll1.selected = -1;
            this.scroll2.selected = -1;
            this.func_73866_w_();
        }
        if (button.field_146127_k == 2 && this.scroll2.hasSelected()) {
            this.scriptUnit.getExternalScripts().remove(this.scroll2.getSelected());
            this.scroll2.selected = -1;
            this.func_73866_w_();
        }
        if (button.field_146127_k == 3) {
            this.scriptUnit.getExternalScripts().clear();
            for (String script : this.scripts) {
                this.scriptUnit.getExternalScripts().add(script);
            }
            this.scroll1.selected = -1;
            this.scroll2.selected = -1;
            this.func_73866_w_();
