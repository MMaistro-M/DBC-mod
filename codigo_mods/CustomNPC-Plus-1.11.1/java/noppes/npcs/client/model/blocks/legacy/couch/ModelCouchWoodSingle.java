/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  net.minecraft.client.model.ModelBase
 *  net.minecraft.client.model.ModelRenderer
 *  net.minecraft.entity.Entity
 */
package noppes.npcs.client.model.blocks.legacy.couch;

import net.minecraft.client.model.ModelBase;
import net.minecraft.client.model.ModelRenderer;
import net.minecraft.entity.Entity;

public class ModelCouchWoodSingle
extends ModelBase {
    ModelRenderer Shape1 = new ModelRenderer((ModelBase)this, 0, 0);
    ModelRenderer Shape2;
    ModelRenderer Shape3;
    ModelRenderer Shape4;
    ModelRenderer Shape5;
    ModelRenderer Shape6;
    ModelRenderer Shape7;
    ModelRenderer Shape8;
    ModelRenderer Shape9;
    ModelRenderer Shape10;
    ModelRenderer Shape11;
    ModelRenderer Shape12;
    ModelRenderer Shape13;
    ModelRenderer Shape14;
    ModelRenderer Shape15;
    ModelRenderer Shape16;
    ModelRenderer Shape17;
    ModelRenderer Shape18;
    ModelRenderer Shape19;
    ModelRenderer Shape20;
    ModelRenderer Shape21;

    public ModelCouchWoodSingle() {
        this.Shape1.field_78809_i = true;
        this.Shape1.func_78789_a(0.0f, 0.0f, 0.0f, 2, 3, 10);
        this.Shape1.func_78793_a(-8.0f, 18.0f, -4.0f);
        this.Shape2 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape2.field_78809_i = true;
        this.Shape2.func_78789_a(0.0f, 0.0f, 0.0f, 2, 10, 2);
        this.Shape2.func_78793_a(-8.0f, 14.0f, -6.0f);
        this.Shape3 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape3.field_78809_i = true;
        this.Shape3.func_78789_a(0.0f, 0.0f, 0.0f, 2, 4, 1);
        this.Shape3.func_78793_a(-8.0f, 14.0f, 0.5f);
        this.Shape4 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape4.func_78789_a(0.0f, 0.0f, 0.0f, 12, 2, 1);
        this.Shape4.func_78793_a(-6.0f, 7.0f, 7.0f);
        this.Shape5 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape5.func_78789_a(0.0f, 0.0f, 0.0f, 12, 2, 2);
        this.Shape5.func_78793_a(-6.0f, 19.0f, 6.0f);
        this.Shape6 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape6.field_78809_i = true;
        this.Shape6.func_78789_a(0.0f, 0.0f, 0.0f, 2, 1, 13);
        this.Shape6.func_78793_a(-8.0f, 13.0f, -7.0f);
        this.Shape7 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape7.func_78789_a(0.0f, 0.0f, 0.0f, 1, 1, 10);
        this.Shape7.func_78793_a(-6.0f, 19.0f, -4.0f);
        this.Shape8 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape8.field_78809_i = true;
        this.Shape8.func_78789_a(0.0f, 0.0f, 0.0f, 2, 18, 2);
        this.Shape8.func_78793_a(-8.0f, 6.0f, 6.0f);
        this.Shape9 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape9.func_78789_a(0.0f, 0.0f, 0.0f, 1, 1, 10);
        this.Shape9.func_78793_a(5.0f, 19.0f, -4.0f);
        this.Shape10 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape10.func_78789_a(0.0f, 0.0f, 0.0f, 1, 10, 1);
        this.Shape10.func_78793_a(-6.0f, 9.0f, 7.0f);
        this.Shape11 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape11.func_78789_a(0.0f, 0.0f, 0.0f, 12, 2, 2);
        this.Shape11.func_78793_a(-6.0f, 19.0f, -6.0f);
        this.Shape12 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape12.func_78789_a(0.0f, 0.0f, 0.0f, 2, 1, 10);
        this.Shape12.func_78793_a(-3.0f, 19.0f, -4.0f);
        this.Shape13 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape13.func_78789_a(0.0f, 0.0f, 0.0f, 2, 1, 10);
        this.Shape13.func_78793_a(1.0f, 19.0f, -4.0f);
        this.Shape14 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape14.func_78789_a(0.0f, 0.0f, 0.0f, 2, 10, 1);
        this.Shape14.func_78793_a(-3.0f, 9.0f, 7.0f);
        this.Shape15 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape15.func_78789_a(0.0f, 0.0f, 0.0f, 2, 10, 1);
        this.Shape15.func_78793_a(1.0f, 9.0f, 7.0f);
        this.Shape16 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape16.func_78789_a(0.0f, 0.0f, 0.0f, 1, 10, 1);
        this.Shape16.func_78793_a(5.0f, 9.0f, 7.0f);
        this.Shape17 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape17.func_78789_a(0.0f, 0.0f, 0.0f, 2, 18, 2);
        this.Shape17.func_78793_a(6.0f, 6.0f, 6.0f);
        this.Shape18 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape18.func_78789_a(0.0f, 0.0f, 0.0f, 2, 1, 13);
        this.Shape18.func_78793_a(6.0f, 13.0f, -7.0f);
        this.Shape19 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape19.func_78789_a(0.0f, 0.0f, 0.0f, 2, 10, 2);
        this.Shape19.func_78793_a(6.0f, 14.0f, -6.0f);
        this.Shape20 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape20.func_78789_a(0.0f, 0.0f, 0.0f, 2, 3, 10);
        this.Shape20.func_78793_a(6.0f, 18.0f, -4.0f);
        this.Shape21 = new ModelRenderer((ModelBase)this, 0, 0);
        this.Shape21.func_78789_a(0.0f, 0.0f, 0.0f, 2, 4, 1);
        this.Shape21.func_78793_a(6.0f, 14.0f, 0.5f);
    }

    public void func_78088_a(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        super.func_78088_a(entity, f, f1, f2, f3, f4, f5);
        this.Shape1.func_78785_a(f5);
        this.Shape2.func_78785_a(f5);
        this.Shape3.func_78785_a(f5);
        this.Shape4.func_78785_a(f5);
        this.Shape5.func_78785_a(f5);
        this.Shape6.func_78785_a(f5);
        this.Shape7.func_78785_a(f5);
        this.Shape8.func_78785_a(f5);
        this.Shape9.func_78785_a(f5);
        this.Shape10.func_78785_a(f5);
        this.Shape11.func_78785_a(f5);
        this.Shape12.func_78785_a(f5);
        this.Shape13.func_78785_a(f5);
        this.Shape14.func_78785_a(f5);
        this.Shape15.func_78785_a(f5);
        this.Shape16.func_78785_a(f5);
        this.Shape17.func_78785_a(f5);
        this.Shape18.func_78785_a(f5);
        this.Shape19.func_78785_a(f5);
        this.Shape20.func_78785_a(f5);
        this.Shape21.func_78785_a(f5);
    }

    private void setRotation(ModelRenderer model, float x, float y, float z) {
        model.field_78795_f = x;
        model.field_78796_g = y;
        model.field_78808_h = z;
    }
}

