...
implements INaturalSpawnsHandler {
    ...
    public SpawnData getRandomSpawnData(String biome, int dimensionId) {
        List<SpawnData> biomeList = this.getSpawnList(biome);
        if (biomeList == null) {
            return null;
        }
        ArrayList<SpawnData> list = new ArrayList<SpawnData>();
        for (SpawnData data : biomeList) {
            if (!data.dimensions.contains(dimensionId)) continue;
            list.add(data);
        }
        if (list.isEmpty()) {
            return null;
    ...
    public INaturalSpawn[] getSpawns() {
        return this.data.toArray(new INaturalSpawn[0]);
    }

    @Override
    public INaturalSpawn[] getSpawns(String biome) {
        List<SpawnData> biomeSpawns = this.getSpawnList(biome);
        if (biomeSpawns == null) {
            return new INaturalSpawn[0];
        }
        return biomeSpawns.toArray(new INaturalSpawn[0]);
    }
