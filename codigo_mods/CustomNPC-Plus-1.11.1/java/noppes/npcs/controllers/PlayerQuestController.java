...
public class PlayerQuestController {
    ...
    public static void addActiveQuest(QuestData questData, EntityPlayer player) {
        ...
        if (PlayerQuestController.canQuestBeAccepted(questData.quest, player)) {
            ...
            if (timeUntilRepeat > 0L && questData.quest.getIsRepeatable() && questData.quest.repeat != EnumQuestRepeat.NONE && questData.quest.repeat != EnumQuestRepeat.REPEATABLE) {
                ...
                player.func_145747_a((IChatComponent)new ChatComponentTranslation(NoppesStringUtils.formatText(message, player), new Object[0]));
            }
        }
    }
    ...
    public static void setQuestFinished(Quest quest, EntityPlayer player) {
        ...
        PlayerQuestController.setQuestFinishedUtil(player, quest, data);
    ...
    public static void setQuestFinishedUtil(EntityPlayer player, Quest quest, PlayerQuestData questData) {
        ...
        if (quest.repeat == EnumQuestRepeat.RLDAILY || quest.repeat == EnumQuestRepeat.RLWEEKLY || quest.repeat == EnumQuestRepeat.RLCUSTOM) {
            ...
            completeTime = player.field_70170_p.func_82737_E();
            questData.finishedQuests.put(quest.id, completeTime);
        }
        if (ConfigMain.ProfilesEnabled && quest.profileOptions.enableOptions && (quest.profileOptions.completeControl == EnumProfileSync.Shared || quest.profileOptions.cooldownControl == EnumProfileSync.Shared)) {
    ...
    public static boolean canQuestBeAccepted(Quest quest, EntityPlayer player) {
        ...
        if (ProfileClientConfig.isProfilesEnabled() && quest.profileOptions.enableOptions && ProfileClientConfig.hasSharedQuest(player, quest.id)) {
        ...
        boolean bl = isFinished = finishedLocally || completedShared;
        ...
        return quest.getTimeUntilRepeat(player) <= 0L;
    }
