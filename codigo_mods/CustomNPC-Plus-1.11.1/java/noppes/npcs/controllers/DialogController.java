    ...
    private DialogCategory loadCategoryDir(File dir) {
        ...
        for (File file : dir.listFiles()) {
            ...
            try {
                ...
                if (dialog.text == null || dialog.text.length() <= 15000) continue;
                LogWriter.error("Extremely big dialog loaded: [ID: " + dialog.id + "] Title: " + dialog.title + " (Length: " + dialog.text.length() + " chars)");
            }
            catch (Exception e) {
