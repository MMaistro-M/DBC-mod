...
public final class AbilityScriptPacket
extends AbstractPacket {
    ...
    public void sendData(ByteBuf out) throws IOException {
        ...
        ByteBufUtils.writeUTF8String((ByteBuf)out, (String)this.abilityId);
    ...
    public void receiveData(ByteBuf in, EntityPlayer player) throws IOException {
        ...
        if (actionOrd < 0 || actionOrd >= Action.values().length) {
            return;
        }
        Action requestedAction = Action.values()[actionOrd];
        String id = ByteBufUtils.readUTF8String((ByteBuf)in);
        ...
        if (requestedAction == Action.GET) {
            ...
            if (ConfigDebug.PlayerLogging && FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
                LogWriter.script(String.format("[%s] (Player) %s SAVED ABILITY SCRIPT %s [%s]", "ABILITY SCRIPTS", player.func_70005_c_(), ability.getName(), ability.getId()));
            }
        }
    }
