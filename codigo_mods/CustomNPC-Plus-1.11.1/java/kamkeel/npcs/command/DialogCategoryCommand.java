...
extends CommandKamkeelBase {
    ...
    public void id(ICommandSender sender, String[] args) throws CommandException {
        if (args.length == 0) {
            ColorUtil.sendError(sender, "Please provide a name for the dialog category");
            return;
        }
        ...
        for (DialogCategory cat : dialogCats) {
            ...
            ColorUtil.sendResult(sender, String.format("Dialog Cat \u00a7e%d\u00a77 - \u00a7c'%s'", cat.id, cat.getName()));
            ++count;
        }
        if (count == 0) {
            ColorUtil.sendResult(sender, String.format("No Dialog Cat found with name: \u00a7c'%s'", catName));
        }
    }
    ...
    public void read(ICommandSender sender, String[] args) throws CommandException {
        ...
        try {
            ...
            ColorUtil.sendError(sender, "DialogCatID must be an integer: " + args[1]);
            return;
        }
        ...
        if (dialogCategory == null) {
            ColorUtil.sendError(sender, "Unknown DialogCatID: " + dialogCatId);
            return;
        }
        int count = 0;
        for (PlayerData playerdata : data) {
            ...
            ColorUtil.sendResult(sender, String.format("Read Dialog Cat \u00a7c'%s' \u00a7e%d\u00a77 for Player '\u00a7b%s\u00a77'", dialogCategory.getName(), dialogCatId, playerdata.playername));
            ColorUtil.sendResult(sender, String.format("Read a total of \u00a7b%d \u00a77dialogs", count));
        }
    }
    ...
    public void unread(ICommandSender sender, String[] args) throws CommandException {
        ...
        for (PlayerData playerdata : data) {
            ...
            playerdata.save();
            playerdata.updateClient = true;
            ColorUtil.sendResult(sender, String.format("Unread Dialog Cat \u00a7c'%s' \u00a7e%d\u00a77 for Player '\u00a7b%s\u00a77'", dialogCategory.getName(), dialogCatId, playerdata.playername));
            ColorUtil.sendResult(sender, String.format("Unread a total of \u00a7b%d \u00a77dialogs", count));
        }
    }
}
