...
public class JsonAnimationUtils {
    ...
    private static JsonElement getObjectByKey(Set<Map.Entry<String, JsonElement>> json, String key) throws GeckoJsonException {
        return (JsonElement)json.stream().filter(x -> ((String)x.getKey()).equals(key)).findFirst().orElseThrow(() -> new GeckoJsonException("Could not find key: " + key)).getValue();
    }

    public static Map.Entry<String, JsonElement> getAnimation(JsonObject animationFile, String animationName) throws GeckoJsonException {
