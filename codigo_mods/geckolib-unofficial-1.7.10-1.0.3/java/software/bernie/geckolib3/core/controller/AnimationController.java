...
public class AnimationController<T extends IAnimatable> {
    ...
    public List emitters = new ArrayList();
    ...
    public void process(double tick, AnimationEvent<T> event, List<IBone> modelRendererList, HashMap<String, Pair<IBone, BoneSnapshot>> boneSnapshotCollection, MolangParser parser, boolean crashWhenCantFindBone) {
        ...
        if (Minecraft.func_71410_x().field_71441_e != null && (time = Minecraft.func_71410_x().field_71441_e.func_82737_E()) > this.lastTick) {
            this.lastTick = time;
            this.updateEmitters();
        }
    ...
    private void processCurrentAnimation(double tick, double actualTick, MolangParser parser, boolean crashWhenCantFindBone) {
        ...
        for (ParticleEventKeyFrame particleEventKeyFrame : this.currentAnimation.particleKeyFrames) {
            if (this.hasParticleOnLoc(particleEventKeyFrame.effect, particleEventKeyFrame.locator) || !(tick >= particleEventKeyFrame.getStartTick()) || this.executedKeyFrames.contains(particleEventKeyFrame)) continue;
            ...
            this.processBedrockParticleEvent((ParticleKeyFrameEvent<T>)event);
    ...
    private void processBedrockParticleEvent(ParticleKeyFrameEvent<T> event) {
        if (BedrockLibrary.instance.presets.containsKey(event.effect)) {
            ...
            emitter.setScheme(GeckoLibMod.particleLibraryInstance.presets.get(event.effect));
            emitter.setTarget((EntityLivingBase)Minecraft.func_71410_x().field_71439_g);
    ...
    public void removeAllStopped() {
        for (int i = 0; i < this.emitters.size(); ++i) {
            if (((BedrockEmitter)this.emitters.get((int)i)).playing || ((BedrockEmitter)this.emitters.get((int)i)).particles.size() != 0) continue;
    ...
    public void setAllStopping() {
        for (int i = 0; i < this.emitters.size(); ++i) {
            ((BedrockEmitter)this.emitters.get(i)).setLastLoop();
        }
    }

    public void updateEmitters() {
        for (int i = 0; i < this.emitters.size(); ++i) {
            BedrockEmitter emitter = (BedrockEmitter)this.emitters.get(i);
            String name = emitter.scheme.name;
            String loc = emitter.locator;
            if (!emitter.scheme.toReload) {
                ((BedrockEmitter)this.emitters.get(i)).update();
                continue;
            }
            emitter.stop();
    ...
    public boolean hasParticleOnLoc(String effect, String locator) {
        for (Object obj : this.emitters) {
            BedrockEmitter emitter = (BedrockEmitter)obj;
            if (emitter.scheme == null || emitter.scheme.name == null || !emitter.scheme.name.equals(effect) || emitter.locator == null || !emitter.locator.equals(locator) || !emitter.isLooping()) continue;
            return true;
        }
        return false;
    }
