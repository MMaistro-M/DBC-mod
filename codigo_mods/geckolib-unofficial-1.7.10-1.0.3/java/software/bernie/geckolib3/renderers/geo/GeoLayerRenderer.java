...
public abstract class GeoLayerRenderer<T extends EntityLivingBase>
implements LayerRenderer<T> {
    ...
    protected static <T extends EntityLivingBase> void renderCopyCutoutModel(ModelBase modelParentIn, ModelBase modelIn, ResourceLocation textureLocationIn, T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float partialTicks, float red, float green, float blue) {
        if (!entityIn.func_82150_aj()) {
            modelParentIn.field_78093_q = modelIn.field_78093_q;
            modelParentIn.field_78091_s = modelIn.field_78091_s;
            ...
            GeoLayerRenderer.renderCutoutModel(modelIn, textureLocationIn, entityIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, 0.0625f, red, green, blue);
        }
    }
