...
implements IGeoRenderer<T> {
    ...
    public GeoEntityRenderer(AnimatedGeoModel<T> modelProvider) {
        ...
        ((DummyVanilaModel)this.field_77045_g).renderer = this;
    }
    ...
    public void func_76986_a(Entity entityObj, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        Pair<Float, Float> rotations = this.calculateRotations(entity, partialTicks, shouldSit);
        ...
        this.applyRotations(entity, f7, rotations.getKey().floatValue(), partialTicks);
        ...
        if (this.modelProvider instanceof IAnimatableModel) {
            this.modelProvider.setLivingAnimations(entity, this.getUniqueID((T)entity), (AnimationEvent)predicate);
        }
        GlStateManager.pushMatrix();
        ...
        for (GeoLayerRenderer<EntityLivingBase> geoLayerRenderer : this.layerRenderers) {
            geoLayerRenderer.render(entity, limbSwing, limbSwingAmount, partialTicks, limbSwing, rotations.getValue().floatValue(), headPitch, renderColor);
        }
    ...
    public Pair<Float, Float> calculateRotations(EntityLivingBase entity, float partialTicks, boolean shouldSit) {
        ...
        return new ImmutablePair<Float, Float>(Float.valueOf(f), Float.valueOf(netHeadYaw));
    }
    ...
    protected void applyRotations(T entityLiving, float ageInTicks, float rotationYaw, float partialTicks) {
        ...
        if (((EntityLivingBase)entityLiving).field_70725_aQ > 0) {
            ...
            GlStateManager.rotate(f * this.func_77037_a((EntityLivingBase)entityLiving), 0.0f, 0.0f, 1.0f);
        } else if ((entityLiving instanceof EntityLiving && ((EntityLiving)entityLiving).func_94056_bM() || entityLiving instanceof EntityPlayer) && ("Dinnerbone".equals(s = ChatFormatting.stripFormatting((String)entityLiving.func_70005_c_())) || "Grumm".equals(s)) && (!(entityLiving instanceof EntityPlayer) || !((EntityPlayer)entityLiving).func_82238_cc())) {
            GlStateManager.translate(0.0, (double)(((EntityLivingBase)entityLiving).field_70131_O + 0.1f), 0.0);
