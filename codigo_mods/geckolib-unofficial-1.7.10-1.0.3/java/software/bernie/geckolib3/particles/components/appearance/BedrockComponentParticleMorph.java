...
IComponentParticleInitialize {
    ...
    public void render(BedrockEmitter emitter, BedrockParticle particle, Tessellator builder, float partialTicks) {
        ...
        double x = Interpolations.lerp(particle.prevPosition.x, particle.position.x, (double)partialTicks);
        double y = Interpolations.lerp(particle.prevPosition.y, particle.position.y, (double)partialTicks);
        double z = Interpolations.lerp(particle.prevPosition.z, particle.position.z, (double)partialTicks);
        Vector3d position = this.calculatePosition(emitter, particle, x, y, z);
        x = position.x;
        y = position.y;
        z = position.z;
        if (!GuiModelRenderer.isRendering()) {
            x -= Interpolations.lerp(camera.field_70169_q, camera.field_70165_t, (double)partialTicks);
            y -= Interpolations.lerp(camera.field_70167_r, camera.field_70163_u, (double)partialTicks);
            z -= Interpolations.lerp(camera.field_70166_s, camera.field_70161_v, (double)partialTicks);
        }
        int combinedBrightness = dummy.func_70070_b(Minecraft.func_71410_x().field_71428_T.field_74281_c);
