...
extends SignatureVisitor {
    ...
    private void checkClassName(String name, String message) {
        if (name == null || name.length() == 0) {
            throw new IllegalArgumentException(CheckSignatureAdapter.stringConcat$0(message));
        }
        for (int i = 0; i < name.length(); ++i) {
            if (".;[<>:".indexOf(name.charAt(i)) == -1) continue;
            throw new IllegalArgumentException(CheckSignatureAdapter.stringConcat$1(message, name));
        }
    }

    private static /* synthetic */ String stringConcat$0(String string) {
        return INVALID + string + " (must not be null or empty)";
    }
    ...
    private void checkIdentifier(String name, String message) {
        if (name == null || name.length() == 0) {
            throw new IllegalArgumentException(CheckSignatureAdapter.stringConcat$2(message));
        }
        for (int i = 0; i < name.length(); ++i) {
            if (".;[/<>:".indexOf(name.charAt(i)) == -1) continue;
            throw new IllegalArgumentException(CheckSignatureAdapter.stringConcat$3(message, name));
        }
    }
