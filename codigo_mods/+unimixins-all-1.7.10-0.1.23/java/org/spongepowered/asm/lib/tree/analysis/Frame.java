...
public class Frame<V extends Value> {
    ...
    public V getLocal(int index) {
        if (index >= this.numLocals) {
            throw new IndexOutOfBoundsException(Frame.stringConcat$0(index));
        }
        return this.values[index];
    }

    private static /* synthetic */ String stringConcat$0(int n) {
        return "Trying to get an inexistant local variable " + n;
    }

    public void setLocal(int index, V value) {
        if (index >= this.numLocals) {
            throw new IndexOutOfBoundsException(Frame.stringConcat$1(index));
        }
        this.values[index] = value;
    }

    private static /* synthetic */ String stringConcat$1(int n) {
        return "Trying to set an inexistant local variable " + n;
    }
    ...
    public void execute(AbstractInsnNode insn, Interpreter<V> interpreter) throws AnalyzerException {
        switch (insn.getOpcode()) {
            ...
            case 58: {
                ...
                if (varIndex <= 0 || (local = this.getLocal(varIndex - 1)) == null || local.getSize() != 2) return;
                this.setLocal(varIndex - 1, interpreter.newEmptyValue(varIndex - 1));
                return;
            }
            case 79: 
            ...
            default: {
                throw new AnalyzerException(insn, Frame.stringConcat$2(insn.getOpcode()));
            }
        }
    }
