...
extends InvokeInjector {
    ...
    protected void injectAtInvoke(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (originalArgs.length == 0) {
            throw new InvalidInjectionException((ISelectorContext)this.info, "@ModifyArg injector " + this + " targets a method invocation " + ((MethodInsnNode)node.getOriginalTarget()).name + "()" + Type.getReturnType(methodNode.desc) + " with no arguments!");
        }
