...
extends InsnExpander {
    ...
    public void process(FlowValue node, FlowPostProcessor.OutputSink sink) {
        ...
        FlowValue cst = new FlowValue(ExpressionASMUtils.getNewType(cstInsn), cstInsn, new FlowValue[0]);
        this.registerComponent(cst, Component.CST, jump);
        node.setInsn(new JumpInsnNode(jumpOpcode, jump.label));
        node.setParents(node.getInput(0), cst);
        this.registerComponent(node, Component.JUMP, jump);
    ...
    public void expand(Target target, InjectionNodes.InjectionNode node, InsnExpander.Expansion expansion) {
        ...
        this.expandInsn(target, node, expansion.registerInsn(Component.CST, new InsnNode(cstOpcode)), expansion.registerInsn(Component.JUMP, new JumpInsnNode(jumpOpcode, jump.label)));
    }
