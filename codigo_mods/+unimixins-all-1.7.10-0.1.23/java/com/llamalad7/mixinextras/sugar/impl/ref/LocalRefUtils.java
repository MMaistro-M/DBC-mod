...
public class LocalRefUtils {
    ...
    public static void generateInitialization(InsnList insns, Type innerType) {
        String refImpl = LocalRefClassGenerator.getForType(innerType);
        insns.add(new MethodInsnNode(182, refImpl, "init", Type.getMethodDescriptor(Type.VOID_TYPE, LocalRefUtils.getErasedType(innerType)), false));
    }

    public static void generateDisposal(InsnList insns, Type innerType) {
        String refImpl = LocalRefClassGenerator.getForType(innerType);
        insns.add(new MethodInsnNode(182, refImpl, "dispose", Type.getMethodDescriptor(LocalRefUtils.getErasedType(innerType), new Type[0]), false));
    ...
    public static void generateUnwrapping(InsnList insns, Type innerType, Runnable load) {
        ...
        load.run();
        insns.add(new MethodInsnNode(185, refInterface, "get", Type.getMethodDescriptor(LocalRefUtils.getErasedType(innerType), new Type[0]), true));
        if (!ASMUtils.isPrimitive(innerType)) {
            insns.add(new TypeInsnNode(192, innerType.getInternalName()));
        }
    }
