...
extends Injector {
    ...
    public WrapOperationInjector(InjectionInfo info) {
        ...
        this.operationType = MixinExtrasService.getInstance().changePackage(Operation.class, Type.getType(CompatibilityHelper.getAnnotation((InjectionInfo)this.info).desc), WrapOperation.class);
    ...
    private Type[] cleanIntLikeArgs(Type[] originalArgs) {
        return new IntLikeBehaviour.MatchArgType(0).transform(this.info, Type.getMethodType(this.returnType, originalArgs), Type.getMethodType(this.returnType, this.methodArgs)).getArgumentTypes();
    }
    ...
    private OperationType newInstantiationOperation(Target target, InjectionNodes.InjectionNode node, StackExtension stack) {
        ...
        node.decorate("mixinextras_wrappedOperation", true);
        return new InstantiationOperation(target, target.addInjectionNode(ASMUtils.findInitNodeFor(target, (TypeInsnNode)newNode)), stack, node);
    }
    ...
    extends MethodCallOperation {
        ...
        void copyNode(InsnList insns, int paramArrayIndex, Consumer<InsnList> loadArgs, Type returnType) {
            ...
            loadArgs.accept(insns);
            ASMUtils.ifElse(insns, this.comparison.copyJump(insns), () -> insns.add(new InsnNode(this.comparison.jumpOnTrue ? 3 : 4)), () -> insns.add(new InsnNode(this.comparison.jumpOnTrue ? 4 : 3)));
        }

        @Override
        void afterHandlerCall(InsnList insns, AbstractInsnNode champion) {
            ASMUtils.ifElse(insns, 154, () -> insns.add(new InsnNode(this.comparison.jumpOnTrue ? 3 : 4)), () -> insns.add(new InsnNode(this.comparison.jumpOnTrue ? 4 : 3)));
            if (!this.isWrapped) {
