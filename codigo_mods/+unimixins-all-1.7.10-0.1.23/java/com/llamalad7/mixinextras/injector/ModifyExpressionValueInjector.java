...
extends Injector {
    ...
    private Type cleanIntLikeType(Type valueType) {
        Type expectedDesc = IntLikeBehaviour.MatchReturnType.INSTANCE.transform(this.info, Type.getMethodType(valueType, valueType), Type.getMethodType(this.returnType, this.methodArgs));
    ...
    private class TargetInfo {
        ...
        public void invokeHandler(Type valueType, InsnList after, StackExtension stack) {
            ...
            if (this.isStringConcat) {
                ...
                stack.extra(1);
                after.add(new MethodInsnNode(182, Type.getInternalName(StringBuilder.class), "toString", Bytecode.generateDescriptor(String.class, new Object[0]), false));
                ...
                }, () -> after.add(new InsnNode(this.comparison.jumpOnTrue ? 3 : 4)), () -> after.add(new InsnNode(this.comparison.jumpOnTrue ? 4 : 3)));
            }
            ModifyExpressionValueInjector.this.invokeHandler(valueType, this.target, after, stack);
            if (this.isStringConcat) {
                after.add(new MethodInsnNode(184, Type.getInternalName(MixinExtrasHooks.class), "replaceContents", Bytecode.generateDescriptor(StringBuilder.class, StringBuilder.class, String.class), false));
            } else if (this.comparison != null) {
