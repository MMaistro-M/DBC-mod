...
extends Injector {
    ...
    public WrapMethodInjector(InjectionInfo info) {
        ...
        this.operationType = MixinExtrasService.getInstance().changePackage(Operation.class, Type.getType(CompatibilityHelper.getAnnotation((InjectionInfo)this.info).desc), WrapMethod.class);
    ...
    private void checkSignature(Target target) {
        ...
        if (sugars != null) {
            for (int i = 0; i < argIndex; ++i) {
                ...
                if (!MixinExtrasService.getInstance().isClassOwned(Type.getType(sugar.desc).getClassName())) continue;
