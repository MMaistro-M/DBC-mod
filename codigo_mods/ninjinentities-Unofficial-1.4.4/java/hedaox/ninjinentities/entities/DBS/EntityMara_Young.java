...
extends EntityDBCNinjin {
    ...
    public void func_70071_h_() {
        ...
        if (this.tickCounter % ModConfig.kiDrainIntervalYoung == 0) {
            List<EntityPlayer> playersInRange = this.getPlayersInRange(ModConfig.kiDrainRangeYoung);
            for (EntityPlayer player : playersInRange) {
                int drainedKi;
                if (player == null || (drainedKi = this.reducePlayerKi(player, ModConfig.kiDrainAmountYoung)) <= 0) continue;
    ...
    private void enhanceEntityWithKi(double ki) {
        ...
        double newMaxHealth = Math.floor(currentMaxHealth + ki * this.parseRatio(ModConfig.kiToMaxHealthRatioYoung, 1.0));
        double newAttackDamage = Math.floor(currentAttackDamage + ki * this.parseRatio(ModConfig.kiToAttackRatioYoung, 1.0));
